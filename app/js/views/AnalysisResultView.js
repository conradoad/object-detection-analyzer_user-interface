export class AnalysisResultView {
    ...
    populateResults(resultMatrix) {
        const base = resultMatrix.reduce((obj1, resDim1) => {
            return resDim1.reduce((obj, result) => {
                ...
                if (qtdFaces < obj.faces.min)
                    obj.faces.min = qtdFaces;
                if (qtdFaces > obj.faces.max)
                    obj.faces.max = qtdFaces;
                if (time < obj.time.min)
                    obj.time.min = time;
                if (time > obj.time.max)
                    obj.time.max = time;
                return obj;
            }, obj1);
        }, {
    ...
    adjustCellsSize() {
        var _a;
        const xAxisLength = (_a = this.analysisResult) === null || _a === void 0 ? void 0 : _a.xAxis.length;
        if (!xAxisLength)
            return;
        const totalWidth = document.querySelector('#matrixResultContainer').clientWidth;
